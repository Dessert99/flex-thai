...
모든 Wave 브랜치는 이전 Wave가 완전히 통합·검증된 최신 local `main`에서
생성한다. local `main`을 원격에 게시한 경우에만 같은 SHA의
`origin/main`을 기준으로 삼는다. 독립성이 없는 작업을 세 번째 슬롯을
채우기 위해 억지로 병렬화하지 않는다.
