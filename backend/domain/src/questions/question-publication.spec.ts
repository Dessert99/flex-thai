  ...
  listRequiredTargets: () => {
    ...
    return Promise.resolve(
      overrides.ttsTargets ?? [
        ...
        { kind: 'THAI_SENTENCE_VERSION', targetId: 'sentence-2', mediaStatus: 'READY' },
      ],
    );
  },
  loadQuestion: () => {
  ...
  it.each(['MISSING', 'UPLOADING', 'FAILED'] as const)(
    ...
    async (mediaStatus) => {
      ...
      const service = createService(
        createTransaction(calls, {
          ttsTargets: [
            { kind: 'THAI_SENTENCE_VERSION', targetId: 'ready-target', mediaStatus: 'READY' },
            { kind: 'VOCABULARY_PRONUNCIATION', targetId: 'blocked-target', mediaStatus },
          ],
        }),
        calls,
      );
