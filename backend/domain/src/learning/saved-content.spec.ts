...
class FakeSavedContentRepository implements SavedContentRepository {
  ...
  readonly savedVocabularyDates: Date[] = [];
  questionAvailable = true;
  vocabularyAvailable = true;
  questionAvailabilityChecks = 0;
  vocabularyAvailabilityChecks = 0;
  ...
  isVocabularyAvailable(): Promise<boolean> {
    this.vocabularyAvailabilityChecks += 1;
  ...
  saveVocabulary(
    ...
    this.savedVocabularyDates.push(savedAtInput);
    return Promise.resolve();
  }
...
describe('SavedContentService 저장 콘텐츠', () => {
  it('공개 문제와 게시 어휘를 중복 없이 저장한다', async () => {
    ...
    await service.saveQuestion(userId, 'question-1', savedAt);
    await service.saveVocabulary(userId, 'vocabulary-1', savedAt);
    await service.saveVocabulary(userId, 'vocabulary-1', savedAt);

    expect([...repository.savedQuestions]).toEqual(['user-1:question-1']);
    expect([...repository.savedVocabularies]).toEqual(['user-1:vocabulary-1']);
    expect(repository.savedQuestionDates).toEqual([savedAt, savedAt]);
    expect(repository.savedVocabularyDates).toEqual([savedAt, savedAt]);
  });

  it('숨긴 문제와 비공개 어휘는 저장하지 않는다', async () => {
    const repository = new FakeSavedContentRepository();
    repository.questionAvailable = false;
    repository.vocabularyAvailable = false;
    const service = new SavedContentService(repository);
    ...
    await expect(
      service.saveVocabulary(userId, 'vocabulary-1', savedAt),
    ).rejects.toMatchObject({ code: 'VOCABULARY_UNAVAILABLE' });
    expect(repository.savedQuestions).toHaveLength(0);
    expect(repository.savedVocabularies).toHaveLength(0);
  });
  ...
  it('삭제는 대상이 숨겨진 뒤에도 반복 성공한다', async () => {
    ...
    repository.vocabularyAvailabilityChecks = 0;

    await service.removeQuestion(userId, 'question-1');
    await service.removeQuestion(userId, 'question-1');
    await service.removeVocabulary(userId, 'vocabulary-1');
    await service.removeVocabulary(userId, 'vocabulary-1');

    expect(repository.savedQuestions).toHaveLength(0);
    expect(repository.savedVocabularies).toHaveLength(0);
    expect(repository.questionAvailabilityChecks).toBe(0);
    expect(repository.vocabularyAvailabilityChecks).toBe(0);
  });
});
